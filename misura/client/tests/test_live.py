...
class KidRegistry(unittest.TestCase):
    ...
    def test_register_different_widgets(self):
        ...
        another_widget = widgets.Active(
            self.root, self.root, self.root.gete('name'))
        registry.register(another_widget)

        self.assertEqual(registry.times.get(key_id, "Value not found"), 0)
        self.assertEqual(
            registry.rid.get(key_id, "Value not found"), [widget, another_widget])
        ...
        self.assertEqual(
            registry.rid.get(key_id, "Value not found"), "Value not found")
        self.assertEqual(
            registry.times.get(key_id, "Value not found"), "Value not found")

    @unittest.skip('')
    def test_update_all(self):
        up = registry.update_all()
        # Using ActiveObjcect cause we need signals
        widget = widgets.ActiveObject(
            self.root, self.root, self.root.gete('name'))
        another_widget = widgets.ActiveObject(
            self.root, self.root, self.root.gete('comment'))
