#!/usr/bin/python
# -*- coding: utf-8 -*-
"""Baseline widget displaying baseline, zero axis and jump axis."""
...
class FlashBaseline(utils.OperationWrapper, veusz.widgets.Axis):
    typename = 'flashBaseline'
    description = 'Flash Baseline'
    ...
    def __init__(self, parent, name=None):
        ...
        if type(self) == FlashBaseline:
            self.readDefaults()

        self.addAction(veusz.widgets.widget.Action('up', self.actionUp,
                                                   descr='Update Baseline',
                                                   usertext='Update Baseline'))


    def draw(self, posn, phelper, outerbounds = None):
        """Override draw() to propagate to children"""
        veusz.widgets.Axis.draw(self, posn, phelper, outerbounds)
    ...
    def up_coord(self, oldx=None, oldy=None, xData=None, yData=None):
        ...
        if (0 in [xRange, yRange]) or np.isnan(xRange) or np.isnan(yRange):
            logging.debug('ERROR: Datapoint divide for ranges', xRange, yRange)
            return
    ...
    def critical_search(self, src):
        ...
        if rg > 0:
            i1 = self.point_index
            xmi = xm[i1]
            logging.debug('searching', xmi, i1, rg)
        ...
        if len(r) == 0:
            ...
            return False
        logging.debug('CRITICAL POINTS', src, r)
        logging.debug('xm1', xm1)
    ...
    def line_points(self, line, ang):
        ...
        if aligned:
            ...
            nr = (xR2 * s2 + yR2 * c2)
            L = np.sqrt(dn / nr)
            logging.debug('convL', L)
        else:
            L = np.sqrt(self.xRange ** 2 + self.yRange ** 2) / 2

        logging.debug('preL', L, c, s)

        logging.debug('ranges', self.xRange, self.yRange)
        logging.debug('rangesL', self.xRange / abs(c), self.yRange / abs(s))
        ...
        logging.debug(line.name, 'L', L, 'xL', xL, 'yL', yL, 
                      'angle', ang * 180 / np.pi)
