...
def validate_tabname(fn):
	...
	fn="".join([x for x in fn if x.isalpha() or x.isdigit() or x not in badchars])
	return fn
...
fields_Intervalli='Tipo_Salita, Inizio, Intervallo, Fine, Inizio1, Intervallo1, Fine1, '	# intervalli
...
def getInstrumentName(tp):
	...
	if tp in [0,1,2,3,4,110,111]: return 'hsm'
	if tp in [6,7,9]: return 'horizontal'
	if tp==etp.DilatometroVerticale: return 'vertical'
	if tp in [5,10,12]: return 'flex'
	if tp==etp.Rodaggio: return 'kiln'
	if tp==etp.DTA: return 'dta'
...
def getHeaderCols(tt, tcode='', all=False):
	...
	if tt in [etp.DilatometroVerticale,  etp.DilatometroOrizzontale, etp.Calibrazione]:
		...
		c+=[fimg.Sint_SX, fimg.Rapporto_DX, fimg.Angolo_SX, fimg.Rapporto_SX,  fimg.Angolo_DX,fimg.Posizione]
	...
	# FLEX
		...
		h+=['Flex', 'S', 'camA', 'P','Mov']
		c+=[fimg.Sint_SX, fimg.Rapporto_DX, fimg.Sint_DX, fimg.Angolo_DX, fimg.Posizione]
	...
	# HSM
		...
		c+=[fimg.Sint_DX, fimg.Angolo_DX, fimg.Rapporto_DX, fimg.Area_DX, fimg.Larghezza2,fimg.Colonna02]
	elif tt==etp.ProvinoL or (tt==etp.ProvinoDoppioCompleto and tcode.endswith('L')):
		h+=['Sint', 'Ang', 'Ratio', 'Area', 'Width','Softening']
		c+=[fimg.Sint_SX, fimg.Angolo_SX, fimg.Rapporto_SX, fimg.Area_SX, fimg.Larghezza,fimg.Colonna01]
...
class TestListModel(QtCore.QAbstractTableModel):
	...
	def data(self, index, role=QtCore.Qt.DisplayRole):
		...
		if role==QtCore.Qt.DisplayRole:
			...
			if len(obj)<=index.column(): return 0
...
def getConnectionCursor(path):
	if 'Linux' in platform.platform():
		#FIXME: Molti valori completamente sballati dalla lettura odbc. Passare a SQLite3?
		cs="DSN=M3DB"
	else:
...
class TestDialog(QtGui.QWidget):
	...
	def __init__(self, parent=None, importOptions=True, path=False):
		...
		self.connect(self.filterType, QtCore.SIGNAL('currentIndexChanged(int)'), self.setFilterType)
		
		# Ricerca per stringa
		self.searchText=QtGui.QLineEdit(self)
		self.connect(self.searchText, QtCore.SIGNAL('returnPressed()'), self.setSearchText)
		...
		# Option to execute a full import to dictionary or only a path|id import:
		self.fullImport=False
#		self.connect(self.table, QtCore.SIGNAL('doubleClicked(QModelIndex)'), self.select)
		self.connect(self.table, QtCore.SIGNAL('entered(QModelIndex)'), self.select)
		self.connect(self.table, QtCore.SIGNAL('clicked(QModelIndex)'), self.preview)
	...
	def selectDB(self):
		...
		path=QtGui.QFileDialog.getOpenFileName(self,"Select Misura3 Database","C:\ESS\Misura3\db")
	...
	def setSearchText(self):
		...
		ntests=[]
		for row in tests:
			if t in row[2].lower(): ntests.append(row)
	...
	def convert(self,path):
		...
		if not os.path.exists(outdir):
			os.mkdir(outdir)
		progress=ProgressBar('Converting to misura file format', dbpath, 'Running')
		progress.bar.setValue(0)
		progress.show()
		progress.connect(progress, QtCore.SIGNAL('jobs(int)'), progress.bar.setMaximum)
		progress.connect(progress, QtCore.SIGNAL('job(int)'), progress.bar.setValue)
	...
	def preview(self, idx=False):
		...
		self.cursor.execute("select * from IMMAGINI where IDProve = '%s'" % code)
...
class ImageDecoder(QtCore.QThread):
	...
	def get(self, path):
		if not path in self.names: return False
		return self.images[path]
...
class ImagePreviewDelegate(QtGui.QItemDelegate):
	...
	def __init__(self, parent=None):
		...
		self.connect(self.decoder, QtCore.SIGNAL('cached(QModelIndex)'),parent.update)
		self.decoder.start()
...
class ImagePreview(QtGui.QTableView):
	...
	def __init__(self, parent=None):
		...
		self.zoomIn=self.menu.addAction('Zoom In', self.itemDelegate().zoomIn)
		self.zoomOut=self.menu.addAction('Zoom Out', self.itemDelegate().zoomOut)
		self.zoomOk=self.menu.addAction('Zoom Reset', self.itemDelegate().zoom)
