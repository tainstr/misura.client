...
# 		self.connect(self.processor, QtCore.SIGNAL('readyFrame(int,int,int,int,int,QByteArray)'), self.updateFrame, QtCore.Qt.QueuedConnection)
	...
	def unscale(self,factor):
		...
		for smp in self.samples:
			smp.unscale(factor)
		r=self.plane.boundingRect() & self.plane.box.boundingRect()  & self.plane.cropBox.boundingRect()
	...
	def addActions(self):
		...
		self.menu.setTitle('Cam %s /dev/%s' % (self.remote['name'], self.remote['dev']))
...
#			cpos={'x':'left','y':'bottom'}
		...
		def add_coord(name):
			...
			submenu.addAction(act)
			self.button_action('limits', label='Search limits',  obj=obj, menu=submenu)
			self.button_action('zero', label='Set zero position',  obj=obj, menu=submenu)
	...
	def add_imaging_actions(self, menu):
		...
		for h in ['exposureTime','contrast', 'brightness', 'gamma', 'gain','saturation','hue']:
			if not self.remote.has_key(h): 
				continue
			act=widgets.aNumberAction(self.server, self.remote, self.remote.gete(h), parent=menu)
			menu.addAction(act)
...
# 					self.reconnectSample()
	...
	def save_frame(self):
		...
		self.saveN += 1
		self.pix.save('%s/%s_%i.jpg' % (self.saveDir, self.remote['name'], self.saveN), 'JPG', 25)
	...
	def reconnectSample(self):
		...
		for i in range(n):
			...
			self.bool_action('Black/white levelling', 'blackWhite', sample.analyzer, m)
			self.bool_action('Adaptive Threshold', 'adaptiveThreshold', sample.analyzer, m)
			self.bool_action('Dynamic Regions', 'autoregion',sample.analyzer, m)			
