...
class SensorPlane(QtGui.QGraphicsItem):
    ...
    def unscale(self, factor):
        ...
        pen = self.pre.pen()
        w = pen.widthF() * f
        logging.debug('Rescaling pens to ', w, factor, f)
        pen.setWidthF(w)
        self.pre.setPen(pen)
    ...
    def pos(self, coord=None, new=None):
        ...
        self.moving[coord] = 1
        logging.debug('moving to', new / self.align, new, self.align)
        return new
    ...
    def motion_start(self):
        ...
        logging.debug('motionStart', self.m_start, self.x(), self.y())
    ...
    def motion_update(self):
        ...
        dx1 = x1 - x
        dy1 = y1 - y
        logging.debug('motion_update', x0, x, x1, y0, y, y1, dx0, dx1)
        self.setPos(-dx0, -dy0)
        ...
        if self.moving['x'] + self.moving['y'] == 0:
            ...
            logging.debug('TIMER STOP', self.moving)
            self.pt.setPos(0, 0)
            self.setPos(0, 0)
    ...
    def motion_stop(self):
        ...
        dx = (self.pt.x())
        dy = (self.pt.y())
        logging.debug('motion_stop', x, dx, y, dy)
        x -= dx
        y -= dy
        self.pos('x', x)
        self.pos('y', y)
        ...
        logging.debug('motionStop', self.m_stop, dx / self.align, dy / self.align)
