...
class BoxRegion(Overlay):
	...
	def __init__(self, parentItem, sample=False, Z=1):
		...
		# ROI Corners
		self.ptul = HookPoint(w=10, h=10, pen=self.pen, Z=self.Z + 1, parent=self)
		self.ptul.setFlag(QtGui.QGraphicsItem.ItemIgnoresTransformations, True)
		self.ptbr = HookPoint(w=10, h=10, pen=self.pen, Z=self.Z + 2, parent=self)
		...
		self.label.setFlag(QtGui.QGraphicsItem.ItemIgnoresTransformations, True)
		self.font=self.label.font()
		
		# Imposto gli hook dei punti in modo che quando vengono mossi provochino il ridimensionamento del rettangolo
		self.ptul.hook = self.reDim
		self.ptul.hookPress = self.blockUpdates
		self.ptul.hookRelease = self.setCurrent	# al rilascio del mouse, i valori vengono applicati sul server
