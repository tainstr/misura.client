...
class SamplePoints(Overlay):
	def __init__(self,parentItem,Z=1):
		...
		for p in self.opt:
			if not p.startswith('i'): continue
	...
	def up(self):
		...
		print 'updating points', self.current.keys(), rx, cx, ry, cy
		d=10
...
class BaseHeight(Overlay):
	def __init__(self,parentItem,Z=1,umpx=1):
		Overlay.__init__(self,parentItem,Z=Z)
		self.opt=set(['profile','iA','roi','angle','w','h','xmass','ymass', 'crop'])
