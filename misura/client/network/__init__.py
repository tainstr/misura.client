...
def simpleConnection(addr,user='',password='',save=True):
	addr=str(addr)
	print 'simpleConnection',addr,user,password
	if '@' in addr:
	...
	if not addr.endswith('/RPC'):
		addr+='/RPC'
	print 'simpleConnection2',addr,user,password
	
	try:
		obj=MisuraProxy(addr,user=user,password=password)
	except:
		print 'FAILED simpleConnection at instantiation'
		print_exc()
		return False, None
	try:
		obj.remObj.echo('echo')
	except ProtocolError as err:
		print 'FAILED simpleConnection at echo'
		print_exc()
		if err.errcode==401:
		...
		print 'FAILED simpleConnection at echo - unknown'
...
def setRemote(obj):
	global manager
	print 'Setting network.manager.remote',repr(obj)
...
def closeConnection():
	...
	manager.remote=None
	print 'Disconnected from', manager.addr
	return True
