...
class TreePanel( QtGui.QSplitter):
	def __init__(self, server=False,parent=None,select=False):
		...
		self.connect(self.view,QtCore.SIGNAL('activated(QModelIndex)'), self.select)
		self.connect(self.view,QtCore.SIGNAL('objTable(QModelIndex)'), self.objTable)
		self.connect(self.view,QtCore.SIGNAL('objNewTab(QModelIndex)'), self.objNewTab)
...
class MConf( QtGui.QMainWindow):
	...
	def __init__(self, server=False,fixed_path=False, parent=None):
		...
		self.connect(self.menu,QtCore.SIGNAL('server_disconnect()'),self.server_disconnect)
		self.connect(self.menu,QtCore.SIGNAL('server_shutdown()'),self.server_shutdown)
