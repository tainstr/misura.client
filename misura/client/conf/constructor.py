...
def orgSections(prop_dict):
	...
	for handle, prop in prop_dict.iteritems():
		if not prop.has_key('readLevel'): prop['readLevel']=-1
		if prop['readLevel']>params.configuration_level: continue
		if ':' in handle: spl=':'
		elif '_' in handle: spl='_'
		else: 
