...
class RemoteFileProxy(object):
	...
	def __init__(self,obj,conf=False,live=None):
		self.obj=obj
		# It is considered live if has the same uid as parent FileServer live uid.
		my=self.obj.get_uid()
		if live is None:
	...
	def get_path(self):
		...
		path='{}//{}:{}@{}|{}'.format(h,self.obj.user,self.obj.password,addr,uid)
		return path
	...
	def __getattr__(self,path):
		...
		# If the function requires a decoding, build the partial decoding function
