...
class TestTermalCycleFlags(unittest.TestCase):
    ...
    def test_should_not_be_editable_when_item_is_not_valid(self):
    	...
    	self.assertEqual(not_editable, thermal_cycle_flags.execute(None, not_valid_index))
    ...
    def test_should_not_be_editable_when_you_are_in_the_temp_column_and_time_is_negative(self):
    	...
    	not_editable = QtCore.Qt.ItemFlags(QtCore.Qt.NoItemFlags)
    	self.assertEqual(not_editable, thermal_cycle_flags.execute(FakeTermalCurveModel(dat), valid_index))
    ...
    def test_temperature_should_be_editable_when_rate_is_zero_for_first_row(self):
    	...
    	self.assertEqual(editable, thermal_cycle_flags.execute(FakeTermalCurveModel(dat), valid_index))
