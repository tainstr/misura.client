...
#		self.mw.document.eval_context[imp]=self.open_file
	...
	def buildMenus(self, actions):
		...
		veusz.utils.constructMenus(self.mw.menuBar(), {self.name.lower():self.menu}, mmenus, actions)
...
class misuraInterface(widgets.Linguist, CustomInterface, QtCore.QObject):
	...
	def __init__(self, mainwindow):
		...
		self.mw.addDockWidget(QtCore.Qt.RightDockWidgetArea, self.openedFilesDock)
...
# 		self.connect(self.mw.plot,QtCore.SIGNAL("sigUpdatePage"),self.update_page)
		self.connect(self.openedFiles,QtCore.SIGNAL("select(QString)"),self.nav_select)
		
		# Recent Files Menus
		self.recentFile=RecentMenu(confdb,'file',self.mw)
		self.connect(self.recentFile,QtCore.SIGNAL('select(QString)'),self.liveImport)
		self.menu.addMenu(self.recentFile)
		self.recentDatabase=RecentMenu(confdb,'database',self.mw)
		self.connect(self.recentDatabase,QtCore.SIGNAL('select(QString)'),self.open_database)
		self.menu.addMenu(self.recentDatabase)
		self.recentServer=RecentMenu(confdb,'server',self.mw)
		self.connect(self.recentServer,QtCore.SIGNAL('select(QString)'),self.open_server)
...
#		print 'Update title',pg.path
	...
	def listImport(self):
		...
		if not getattr(self, 'imd', False): return 
