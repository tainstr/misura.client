...
# 			fn = veusz.utils.BoundCaller(self.actionSetTimeout, intv)
	...
	def edit_properties(self,pos,frm=False):
		...
		self.w=treeeditwindow.PropertyList(self.document,showformatsettings=frm)
...
class VeuszPlot(QtGui.QWidget):
	...
	def set_doc(self,doc=False):
		...
		self.plot.disconnect(zp, QtCore.SIGNAL('triggered()'), self.plot.slotViewZoomPage)
...
#		self.plot.delayed.singleShot(100, self.delayedUpdate)
	...
	def fitSize(self, zoom=False):
		...
		g=page.name
		if g=='time': g='/time/time'
		elif g=='temperature': g='/temperature/temp'
		if h<2 or w<4:
