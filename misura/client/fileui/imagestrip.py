...
class ImageStrip(QtGui.QWidget):
	...
	def chLen(self):
		n=QtGui.QInputDialog.getInt(self,"New length","Change number of images:",self.n,1,50)
		if n[1]:
			self.setLen(n[0])
	def chRows(self):
		n=QtGui.QInputDialog.getInt(self,"Set rows","Change number of rows displayed:",self.rows,1,10)
		if n[1]:
			self.rows=n[0]
			self.setLen(self.n)	
	def by_index(self):
		val,st=QtGui.QInputDialog.getInt(self,"Step by index","Display one image every N:",value=self.step,min=1,step=1)
		if not st: return 
		self.step=val
		self.bytime=False
		self.set_idx()			
	def by_time(self):
		...
		val,st=QtGui.QInputDialog.getInt(self,"Step by time","Display one image every N seconds:",value=self.step,min=1,step=1)
		if not st: return 
		self.step=val
		self.bytime=True
		self.set_idx()
	...
	def setLen(self,n=-1):
		...
		self.connect(self.labels[-1],QtCore.SIGNAL('set_time(float)'),self.emitSetTime)
		self.set_idx()
...
class Slider(QtGui.QWidget):
	...
	def __init__(self,parent=None):
		...
		self.setLayout(self.lay)
		self.connect(self.cbPath,QtCore.SIGNAL('currentIndexChanged(int)'),self.choice)
	...
	def reset(self,choice=False):
		fp=self.doc.proxy
		if not fp: return
		self.disconnect(self.cbPath,QtCore.SIGNAL('currentIndexChanged(int)'),self.choice)
...
class ImageSlider(QtGui.QWidget):
	def __init__(self,parent=None):
		...
		self.connect(self.strip,QtCore.SIGNAL('set_time(float)'),self.emitSetTime)
		self.connect(self, QtCore.SIGNAL('customContextMenuRequested(QPoint)'),self.strip.showMenu)
		...
		self.connect(self.stripArea, QtCore.SIGNAL('customContextMenuRequested(QPoint)'),self.strip.showMenu)
		
		# Slider for image navigation
		self.slider=Slider(self)
		self.slider.slider.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
		self.connect(self.slider.slider, QtCore.SIGNAL('customContextMenuRequested(QPoint)'),self.strip.showMenu)
#		self.connect(self.slider,QtCore.SIGNAL('set_idx(int)'),self.set_idx)
		...
		self.connect(self.slider,QtCore.SIGNAL('datapathChanged(QString)'),self.setPath)
