...
class Autoupdater(QtCore.QObject):
    ...
    def __init__(self, callback=False, menu=False, timer=False):
        ...
        self.timer.connect(
            self, QtCore.SIGNAL('timeout()'), QtGui.qApp.processEvents)
        if callback:
            self.timer.connect(
                self.timer, QtCore.SIGNAL('timeout()'), callback)
        else:
    ...
    def interval(self):
        from misura.client import conf
        w = QtGui.QInputDialog.getDouble(
            self, "Update interval", "Seconds:", registry.interval, 0.05, 5, 2)
        if w[1]:
            w = w[0] * 1000
