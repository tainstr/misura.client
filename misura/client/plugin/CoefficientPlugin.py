...
class CoefficientPlugin(plugins.DatasetPlugin):
	...
	def __init__(self,ds_x='',ds_y='',start=50., percent=0., reconfigure='Continue',smooth=5,smode='X and Y',ds_out='coeff'):
		...
		self.fields = [
			...
			plugins.FieldFloat('percent', descr='From percentile factor', default=percent),
			plugins.FieldCombo('reconfigure', descr='When cooling is found',items=['Continue','Restart','Stop'],default=reconfigure),
			plugins.FieldInt('smooth','Smoothing Window',default=smooth),
			plugins.FieldCombo('smode', descr='Apply Smoothing to',items=['X and Y','Y alone','Output'],default=smode),
			plugins.FieldDataset('ds_out', 'New output dataset name',default=ds_out),
		]
	...
	def getDatasets(self, fields):
		...
		if fields['ds_x']==fields['ds_y']:
			raise plugins.DatasetPluginException('X and Y datasets cannot be the same')
