...
#		print 'Diff sign',crossing,np.abs(crossing).sum()
		...
		for c in crossings:
			...
			if self.settings.showLabels==True:
				...
				self.ops.append(document.OperationWidgetAdd(self.parent,'label',name=lblname))
			self.ops.append(document.OperationWidgetAdd(self.parent,'datapoint',**dpset))	
		...
		for xy,fd in self.crossings.iteritems():
			for c, (name,dpx,dpy) in fd.iteritems():
				...
				if dp.labelwidget is not None:
					self.ops.append(document.OperationWidgetDelete(dp.labelwidget))
