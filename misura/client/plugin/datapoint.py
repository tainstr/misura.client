...
class DataPoint(utils.OperationWrapper,veusz.widgets.BoxShape):
	...
	def addSettings(klass, s):
		...
		s.add( setting.ChoiceSwitch(
				'search', ['None','Maximum','Minimum','Inflection','Stationary'],'None',
	...
	def dependencies(self):
		...
		if labelwidget is None:
			if s.showLabel:
				name='lbl_'+self.name
				self.ops.append(document.OperationWidgetAdd(self.parent,'label',name=name))
	...
	def distance(self,x,y,xData=None,yData=None,xRange=None,yRange=None):
		...
		if xRange is None: xRange=self.xRange
		if yRange is None: yRange=self.yRange
	...
	def up_coord(self,oldx=None,oldy=None):
		...
		ox_set=s.get('xPos')
		oy_set=s.get('yPos')
		if oldx is None: oldx=ox_set.get()
	...
	def critical_search(self):
		...
		if src=='Stationary':
			sp=interpolate.LSQUnivariateSpline(xm1,ym1,xm1[step:-step:step],k=4)
	...
	def line_points(self,line,ang):
		...
		if aligned:
			...
			c2=c**2; s2=s**2
			c02=np.cos(ang0)**2; s02=np.sin(ang0)**2
			xR2=self.xRange**2; yR2=self.yRange**2
	...
	def updateOutputLabel(self):
		...
		xsig=5
		if self.x/self.xRange>0.7: xsig=-15
		ysig=2
		if self.y/self.yRange>0.7: ysig=-4
