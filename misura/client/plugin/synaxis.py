...
class SynAxis(utils.OperationWrapper, veusz.widgets.Axis):
	...
	def __init__(self, parent, name=None):
		...
		#TODO: enable restore also for Axis translations by recovering original axis coupling
	...
	def actionUp(self):
		...
		for i in range(self.ncurves):
			...
			if None in [xax,yax]: continue
			# Obtain real position relative to xax
			pos=apos*(xax.plottedrange[1]-xax.plottedrange[0])+xax.plottedrange[0]
			...
			# Create new one if current obj happens to share its axis with a previous one
			if axmap.has_key(yax.name):
				...
				self.ops.append(document.OperationWidgetAdd(self.parent,'axis',**axset))
				self.toset(obj,'yAxis',axname)
				axmap[axname]=obj
				self.axmap[obj.path]=(axname,yax.name,(yax.settings.min,yax.settings.max))
# 			else:
