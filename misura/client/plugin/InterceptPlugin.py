...
class InterceptPlugin(utils.OperationWrapper,plugins.ToolsPlugin):
	...
	def __init__(self,target=[],axis='X', val=0., search='None', searchRange=25, text='Intercept\\\\%(xlabel)s=%(x)i\\\\%(ylabel)s=%(y)E'):
		"""Make list of fields."""
		self.fields = [ 
			plugins.FieldDatasetMulti("target", descr="Datasets whose curves to intercept", default=target),
			plugins.FieldCombo("axis",descr="Intercept on X or Y axis",items=['X','Y'],default=axis),
			plugins.FieldFloat('val', 'Value', default=val),
			plugins.FieldCombo("search",descr="Place nearest",items=['None','Maximum','Minimum','Inflection','Stationary'],default=search),
			plugins.FieldFloat('searchRange', descr='Nearest search range', default=searchRange),
...
#		targetds=target
		...
		for obj in g.children:
			...
			if len(i) == 0:
				raise plugins.ToolsPluginException('Impossible to find required value: %E' % val)
			i = i[0]
			# Add the datapoint
			cmd.To(g.path)
			name = g.createUniqueName('datapoint_' + obj.name) if not basename else basename + '_' + obj.name
			lblname = name + '_lbl'
...
# 			doc.applyOperation(document.OperationMultiple(self.ops, descr='intercept'))
			# Call the update action in order to correctly position the datapoints
