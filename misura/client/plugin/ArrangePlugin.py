...
class ArrangePlugin(utils.OperationWrapper,plugins.ToolsPlugin):
	...
	def __init__(self,dataset='Line Color', sample='Line Style', space=False):
		...
		self.fields = [ 
			plugins.FieldCombo("dataset", descr="Datasets marking mode:", items=defvars.keys(),default=dataset),
			plugins.FieldCombo("sample", descr="Samples marking mode:", items=defvars.keys(),default=sample),
			plugins.FieldBool("space", descr="Axis positioning:", default=space)
		]
	...
	def apply(self, cmd, fields):
		...
		if fields['sample']==fields['dataset']:
			raise plugins.ToolsPluginException('You must choose different markers for Datasets and Samples.')
		...
		for plotpath in tree['plot']:
			...
			# Set plot colors equal to their yaxis
			...
			# Get output value
			idx=0
			if smp in smps: idx=smps.index(smp)
			if m_var=='m_marker':
