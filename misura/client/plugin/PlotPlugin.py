...
class PlotDatasetPlugin(utils.OperationWrapper,plugins.ToolsPlugin):
	...
	def initCurve(self, name='Curve', xData='xD', yData='yD',
		...
		self.toset(yax,'label', axisLabel.replace('_','\\_').replace('/','.'))
...
#			print 'NOT CONVERTING',ds.name(),cvt,pc,dslist,tree['axis'],ax_name
	...
	def apply(self, cmd, fields):
		...
		for i,x in enumerate(valid_x):
			...
			# Color index
			idx=0
			if ds.m_var in default_curves: idx=default_curves.index(ds.m_var)
		...
		for gname in gnames:
			...
			self.ops.append(document.OperationToolsPlugin(ArrangePlugin(),fields.copy()))
...
class DefaultPlotPlugin(plugins.ToolsPlugin):
	...
	def apply(self, cmd, fields):
		...
		for ds in vars:
			if timeds: 
				xt.append(timeds); yt.append(ds)
			if tempds: 
				xT.append(tempds); yT.append(ds)
		# Plot also T over t
