...
class MainWindow(QtGui.QMainWindow):
	...
	def __init__(self, parent=None):
		...
		self.connect(self.tab,QtCore.SIGNAL('tabCloseRequested(int)'),self.close_tab)
		
		self.connect(self.myMenuBar.recentFile,QtCore.SIGNAL('select(QString)'),self.open_file)
		
		self.connect(self.myMenuBar.recentDatabase,QtCore.SIGNAL('select(QString)'),self.open_database)
		self.connect(self.myMenuBar,QtCore.SIGNAL('new_database(QString)'),self.new_database)
		
		self.connect(self.myMenuBar.recentServer,QtCore.SIGNAL('select(QString)'),self.open_server)
		
		self.connect(self.myMenuBar.recentM3db,QtCore.SIGNAL('select(QString)'),self.open_m3db)
		...
		self.connect(greeter.database,greeter.database.sig_select,self.open_database)
		self.connect(greeter.server,greeter.server.sig_select,self.open_server)
	...
	def open_database(self,path,new=False):
		...
		self.connect(idb,QtCore.SIGNAL('selectedFile(QString)'),self.open_file)
	...
	def open_server(self,addr):
		...
		if not idb: return False
		win=self.area.addSubWindow(idb)
		win.show()
		self.connect(idb,QtCore.SIGNAL('selectedRemoteUid(QString,QString)'),self.open_remote_uid)
		return True
